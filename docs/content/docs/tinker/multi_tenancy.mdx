...
When sampling is forwarded to non-colocated SkyRL-Train inference, stale-session auto-cleanup is disabled as a fail-safe. The engine subprocess cannot observe the API process's in-flight forwarding tasks, so deleting an adapter from the engine would race those requests. Clients must explicitly unload their models; if a client disappears before doing so, restart the service to reclaim its adapter rather than relying on heartbeat expiry. Colocated and JAX-backed servers retain automatic stale-session cleanup.
