...
# Colocated GRPO training+generation for Qwen3-0.6B on GSM8K with Megatron and PiSSA.
...
uv run --isolated --extra megatron -m skyrl.train.entrypoints.main_base \
  ...
  trainer.run_name="gsm8k_megatron_tp${MEGATRON_TP}_pp${MEGATRON_PP}_cp${MEGATRON_CP}_${SOURCE_MODEL}_pissa_r${LORA_RANK}_a${LORA_ALPHA}" \
  trainer.resume_mode=from_path \
  trainer.resume_path="$PISSA_INIT_DIR/global_step_0" \
  trainer.ckpt_path="$HOME/ckpts/gsm8k_0.6b_pissa_ckpt" \
  $@
