...
def prepare_inkling_config(
    ...
    *,
    ...
    """Validate the supported training layout before allocating model weights."""
    if config.model_type != "inkling_mm_model":
        return
    if language_model_only:
        raise ValueError("Inkling requires the native multimodal model; language_model_only is unsupported")
    if remove_microbatch_padding or sequence_parallel_size != 1:
        raise ValueError("Inkling compact attention requires unpacked training and sequence_parallel_size=1")
    if config.text_config.attention_dropout != 0.0:
        raise ValueError("Inkling compact attention requires zero attention dropout")
    if lora_rank > 0:
        validate_inkling_lora_targets(config, target_modules)
    correct_inkling_expert_size(config, model_path, config_kwargs)
    prepare_inkling_precision(config)


def patch_inkling_model(model):
    """Install Inkling's precision and compact relative-attention patches."""
    if model.config.model_type != "inkling_mm_model":
        return
    from .attention import install_inkling_flex_attention

    install_inkling_precision(model)
    install_inkling_flex_attention(model)


def shard_inkling_fp32_modules(model, fsdp_kwargs):
    """Keep convolution and router computation in separate FP32 FSDP groups."""
    if model.config.model_type != "inkling_mm_model":
        return
