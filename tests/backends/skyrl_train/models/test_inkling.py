...
from skyrl.backends.skyrl_train.patches.inkling.attention import (
...
def test_llama_wrapper_and_fsdp_do_not_import_or_apply_inkling_patches(tmp_path):
    subprocess.run(
        [
            sys.executable,
            "-c",
            inspect.cleandoc(
                ...
                native = AutoModelForCausalLM.from_config(LlamaConfig(
                    vocab_size=32, hidden_size=16, intermediate_size=32,
                    num_hidden_layers=1, num_attention_heads=2, num_key_value_heads=2,
                    tie_word_embeddings=False,
                ), dtype=torch.bfloat16, attn_implementation="sdpa").eval()
                native.save_pretrained(sys.argv[1])
                original_forward = LlamaDecoderLayer.forward
                with patch.dict(sys.modules, {
                    "skyrl.backends.skyrl_train.patches.inkling": None,
                    "transformers.models.inkling": None,
                }):
                    ...
                    torch_utils.FLASH_ATTN_CROSS_ENTROPY_LOSS_AVAILABLE = False
                    wrapper = HFModelWrapper(sys.argv[1], bf16=True).eval()
                    tokens = torch.tensor([[1, 2, 3, 4]])
                    mask = torch.ones_like(tokens)
                    scores, output = wrapper(tokens, 3, attention_mask=mask, return_output=True)
                    expected = native(tokens, attention_mask=mask, use_cache=False).logits
                    torch.testing.assert_close(output.logits, expected, rtol=0, atol=0)
                    (-scores.mean()).backward()
                    assert torch.isfinite(wrapper.model.lm_head.weight.grad).all()
                    assert LlamaDecoderLayer.forward is original_forward
                    assert all(p.dtype == torch.bfloat16 for p in wrapper.model.parameters())
                    policy = fsdp_utils.MixedPrecisionPolicy(
                        param_dtype=torch.bfloat16, reduce_dtype=torch.float32,
                    )
                    with patch.object(fsdp_utils, "fully_shard") as shard:
                        fsdp_utils.apply_fsdp2(wrapper.model, {"mp_policy": policy}, FSDPConfig())
                    assert [type(call.args[0]).__name__ for call in shard.call_args_list] == [
                        "Embedding", "LlamaDecoderLayer", "LlamaForCausalLM",
                    ]
                    assert all(call.kwargs == {"mp_policy": policy} for call in shard.call_args_list)
                """
            ),
            str(tmp_path),
        ],
...
@pytest.mark.parametrize("meta_init", [False, True])
@pytest.mark.parametrize(
    "options",
    [
        {"remove_microbatch_padding": True, "use_flash_attention_2": True},
        {"sequence_parallel_size": 2},
    ],
)
def test_unsupported_inkling_layout_fails_before_model_allocation(tmp_path, meta_init, options):
    from skyrl.backends.skyrl_train.workers.model_wrapper import HFModelWrapper

    tiny_config().save_pretrained(tmp_path)
    with (
        patch.object(AutoModelForImageTextToText, "from_pretrained") as load,
        patch.object(AutoModelForImageTextToText, "from_config") as allocate,
        pytest.raises(ValueError, match="Inkling"),
    ):
        HFModelWrapper(str(tmp_path), meta_init=meta_init, **options)
...
def test_published_expert_width_controls_loaded_tensor_shape(tmp_path):
    ...
    assert model.model.language_model.layers[1].mlp.experts.gate_up_proj.shape == (
        4,
        32,
        32,
    )
...
def test_router_precision_survives_checkpoint_meta_init_and_shared_expert_backward(
    tmp_path,
):
    from skyrl.backends.skyrl_train.workers.model_wrapper import HFModelWrapper
    ...
    for model in (loaded, meta):
        assert model.config.text_config._attn_implementation == "skyrl_inkling_flex_attention"
    ...
    with torch.no_grad():
        ...
        for left, right in [(0, 3), (3, 0), (2, 2)]:
            ...
            observed = sdpa(
                ...
                use_cache=False,
            ).logits[:, left : left + tokens.shape[1]]
...
def test_fsdp2_bf16_lora_preserves_native_fp32_convolutions(tmp_path):
            ...
            if isinstance(module, modeling_inkling.InklingShortConvolution):
                ...
                module.register_forward_hook(
                    lambda module, inputs, output: router_observed.append(
                        (
                            module.weight.dtype,
                            module.e_score_correction_bias.dtype,
                            inputs[0].dtype,
                            output[1].dtype,
                        )
                    )
...
@pytest.mark.skipif(
    ...
    reason="FSDP2 initialization regression requires two CUDA ranks",
)
...
def test_attention_adapter_export_preserves_factors_and_uses_published_names():
    model = get_peft_model(
        ...
        LoraConfig(
            task_type=TaskType.CAUSAL_LM,
            r=4,
            target_modules=["q_proj", "k_proj", "v_proj", "r_proj", "o_proj"],
        ),
    )
    ...
    assert exported_config["target_modules"] == [
        "wq_du",
        "wk_dv",
        "wv_dv",
        "wr_du",
